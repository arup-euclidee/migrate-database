...
function generateMigrationSQL(diff) {
    ...
    for (const [table, changes] of Object.entries(diff.modifiedTables)) {
        ...
        for (const col of changes.addedColumns) {
            const newDef = changes.changedColumns.find((c) => c.name === col)?.newDef;
            if (newDef) {
                sql.push(`ALTER TABLE "${table}" ADD COLUMN "${newDef.name}" ${newDef.type}${newDef.nullable === false ? " NOT NULL" : ""}${newDef.defaultValue ? ` DEFAULT ${newDef.defaultValue}` : ""};`);
            }
        }
        ...
        for (const colChange of changes.changedColumns) {
            sql.push(`ALTER TABLE "${table}" ALTER COLUMN "${colChange.name}" TYPE ${colChange.newDef.type};`);
            ...
            if (colChange.oldDef.defaultValue !== colChange.newDef.defaultValue) {
                if (colChange.newDef.defaultValue) {
                    sql.push(`ALTER TABLE "${table}" ALTER COLUMN "${colChange.name}" SET DEFAULT ${colChange.newDef.defaultValue};`);
                }
                else {
        ...
        for (const idx of changes.addedIndexes) {
            sql.push(`CREATE ${idx.unique ? "UNIQUE " : ""}INDEX "${idx.name}" ON "${table}" (${idx.columns
                .map(c => `"${c}"`)
                .join(", ")});`);
        }
        for (const idx of changes.changedIndexes) {
            sql.push(`DROP INDEX IF EXISTS "${idx.oldDef.name}";`);
            sql.push(`CREATE ${idx.newDef.unique ? "UNIQUE " : ""}INDEX "${idx.newDef.name}" ON "${table}" (${idx.newDef.columns
                .map(c => `"${c}"`)
                .join(", ")});`);
        }
        // Constraints
        ...
        for (const c of changes.addedConstraints) {
            if (c.type === "unique") {
                sql.push(`ALTER TABLE "${table}" ADD CONSTRAINT "${c.name}" UNIQUE (${c.columns
                    .map(c => `"${c}"`)
                    .join(", ")});`);
            }
            else if (c.type === "primary") {
                sql.push(`ALTER TABLE "${table}" ADD CONSTRAINT "${c.name}" PRIMARY KEY (${c.columns
                    .map(c => `"${c}"`)
                    .join(", ")});`);
            }
        }
        for (const c of changes.changedConstraints) {
            ...
            if (c.newDef.type === "unique") {
                sql.push(`ALTER TABLE "${table}" ADD CONSTRAINT "${c.newDef.name}" UNIQUE (${c.newDef.columns
                    .map(c => `"${c}"`)
                    .join(", ")});`);
            }
        }
        ...
        for (const rel of changes.addedRelations) {
            const fkName = `fk_${table}_${rel.columns.join("_")}`;
            sql.push(`ALTER TABLE "${table}" ADD CONSTRAINT "${fkName}" FOREIGN KEY (${rel.columns
                .map(c => `"${c}"`)
