...
exports.validateCrossTableRelations = validateCrossTableRelations;
const validation_rules_1 = require("./validation-rules");
function validateCrossTableRelations(schema, context) {
    const errors = [];
    const warnings = [];
    schema.forEach((table) => {
        if (!table.relations)
            return;
        table.relations.forEach((relation, relationIndex) => {
            ...
            for (let i = 0; i < relation.columns.length; i++) {
                ...
                if (!sourceColumns[sourceCol])
                    continue;
                if (!targetColumns[targetCol])
                    continue;
            ...
            const isTargetPrimaryKey = reference.columns.every(col => {
                const column = targetColumns[col];
...
function areTypesCompatible(sourceType, targetType) {
    if (sourceType === targetType)
        return true;
    const compatibleTypes = validation_rules_1.TYPE_COMPATIBILITY[sourceType] || [];
    return compatibleTypes.includes(targetType);
}
function extractSize(typeString) {
    const match = typeString.match(/\((\d+)\)/);
    return match ? parseInt(match[1]) : null;
}
function findCircularDependencies(schema, context) {
    const errors = [];
    const graph = new Map();
    ...
    function detectCycle(tableName) {
