...
export class MigrationGenerator {
    static generateMigrationSQL(currentSchema: SchemaDefinition, previousSchema: SchemaDefinition | null): string {
        ...
        if (!previousSchema) {
            ...
            if (record?.newTables?.length > 0) {
                sql += this.generateCreateTableSQL(record.newTables);
            }
            // existing tables
    ...
    private static compareSchemas(oldSchema: SchemaDefinition, newSchema: SchemaDefinition) {
        ...
        return {
            removedTables,
            newTables,
            existingCommonTables,
        };
    }
    private static compareTables(oldSchema: SchemaDefinition, newSchema: SchemaDefinition, tables) {
        const oldTables = new Map(oldSchema.tables.map(t => [t.name, t]));
        const newTables = new Map(newSchema.tables.map(t => [t.name, t]));

        for (const table of tables) {
            const oldTable = oldTables.get(table);
            const newTable = newTables.get(table);

            const columnSteps = this.compareColumns(oldTable, newTable);
            // const indexSteps = this.compareIndexes(oldTable, newTable);
            // return [...columnSteps, ...indexSteps];
        }
    }

    private static compareColumns(oldTable: TableDefinition, newTable: TableDefinition) {
        const steps: MigrationStep[] = [];
    ...
    private static generateCreateTableSQL(tables) {
        let create_sql = '';
        for (const table of tables) {
            ...
            if (table.indexes) {
                for (const index of table.indexes) {
                    const unique = index.unique ? 'UNIQUE ' : '';
                    create_sql += `CREATE ${unique}INDEX ${index.name} ON ${table.name} (${index.columns.join(', ')});\n`;
                }
                create_sql += '\n';
            }
        }
